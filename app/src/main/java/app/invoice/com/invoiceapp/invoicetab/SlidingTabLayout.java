...
package app.invoice.com.invoiceapp.invoicetab;
...
public class SlidingTabLayout extends HorizontalScrollView {
	...
	public SlidingTabLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		mTabStrip.setBackgroundColor(context.getResources().getColor(
				R.color.black));
	...
	private class InternalViewPagerListener implements
		...
		public void onPageSelected(int position) {
			...
			View v = mTabStrip.getChildAt(position);
	...
	private class TabClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			for (int j = 0; j < mTabStrip.getChildCount(); j++) {
				...
				// "Title:"+((TextView)mTabStrip.getChildAt(j)).getText(),
				// Toast.LENGTH_SHORT).show();
				if (v == mTabStrip.getChildAt(j)) {
					((TextView) v).setTextColor(Color.WHITE);
				} else {
					((TextView) mTabStrip.getChildAt(j))
							.setTextColor(Color.GRAY);
				}
			}
